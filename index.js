...
function renderSelectTab(container) {
    ...
    const previewPanel = $(`<div class="sc-preview-panel">
        ...
        <div id="style-combinator-stats" class="sc-preview-stats"></div>
 ...
 * 한글: 글자당 ~2.5 토큰, 영문/기타: ~4글자당 1토큰
 * 이는 단순 추정치이며 실제 토큰 수는 사용하는 토크나이저에 따라 다를 수 있습니다.
 * @param {string} text
 * @returns {number}
 */
function estimateTokenCount(text) {
    ...
    return Math.round(koreanChars * 2.5 + otherChars / 4);
}
...
function updatePromptInjection() {
    ...
    if (selected.length > 0) {
        ...
        // 모든 STEP 실행 전에 먼저 preamble 확보
        // 우선순위: 톤 > 서사 > 서술 > 세계 > 모드
        const preamblePriority = [
            ...toneStyles, ...narrativeRelStyles, ...narrativeGenStyles,
            ...narrationStyles, ...worldBgStyles, ...worldStStyles, ...modeStyles
        ];
        for (const s of preamblePriority) {
            const { preamble } = parseModules(resolveStylePrompt(s));
            if (preamble) { basePreamble = preamble; break; }
        }
    ...
    const statsEl = $('#style-combinator-stats');
    if (statsEl.length && finalPrompt) {
        const charCount = finalPrompt.length;
        const tokenEstimate = estimateTokenCount(finalPrompt);
        statsEl.text(`📊 ${charCount.toLocaleString()}자 / ~${tokenEstimate.toLocaleString()} 토큰 (추정)`).show();
    } else if (statsEl.length) {
        statsEl.text('').hide();
    }
}
